...
function findEquipo(db,id){
  const v=(id||'').trim();
  return db.equipos.find(e=>e.id===v)||null;
}
...
function parseConexiones(rows) {
  ...
  if(hasHeader){
    header.forEach((h,i)=>{
      ...
      if(k==='equipoid'||k==='equipo') iEqId=i;
      else if(k==='id'||k==='puerto'||k==='idpuerto') iId=i;
      else if(k==='tipo'||k==='type') iTipo=i;
      else if(k==='estado'||k==='status') iEst=i;
      else if(k==='destino'||k==='destination'||k==='dest') iDest=i;
    });
  }
  return dataRows
