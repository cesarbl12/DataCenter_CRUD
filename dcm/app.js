...
function renderEquipoList(filter) {
  ...
  if (_eqFilter) { const q=_eqFilter.toLowerCase(); list=list.filter(e=>safe(e.id).includes(q)||safe(e.modelo).includes(q)||safe(e.numeroSerie).includes(q)||safe(e.servicio).includes(q)||safe(e.estado).includes(q)); }
...
window.filterEquipos    = function(){ _eqFilter=document.getElementById('searchEq').value; renderEquipoList(); };
window.clearEquipoFilter= function(){ _eqFilter=''; document.getElementById('searchEq').value=''; renderEquipoList(); };
...
function renderConexList(filter) {
  ...
  if (_connFilter) { const q=_connFilter.toLowerCase(); list=list.filter(c=>safe(c.id).includes(q)||safe(c.tipo).includes(q)||safe(c.estado).includes(q)||safe(c.destino).includes(q)); }
...
window.filterConexiones = function(){ _connFilter=document.getElementById('searchConn').value; renderConexList(); };
window.clearConnFilter  = function(){ _connFilter=''; document.getElementById('searchConn').value=''; renderConexList(); };
